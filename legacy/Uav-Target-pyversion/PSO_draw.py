...
plt.savefig("D:\\西工大\\2024秋\\大论文\\图库\\第三章\\svg\\pso目标集划分.svg", dpi=600, format="svg")
plt.grid()
plt.show()
...
plt.xlabel('迭代轮数')
plt.ylabel('适应度函数值')
plt.savefig("D:\\西工大\\2024秋\\大论文\\图库\\第三章\\svg\\收敛曲线.svg", dpi=600, format="svg")
plt.grid()
plt.legend()
plt.show()
...
for cluster_index in given_clusters:
    ...
    plt.scatter(cluster_points[:, 1]*10, cluster_points[:, 2]*10, label=f'Cluster {cluster_index}', s=50)
    for index in indices:
        plt.annotate(int(index), (cluster_points[indices.index(index), 1]*10, cluster_points[indices.index(index), 2]*10),
                     fontsize=8, ha='center', va='bottom')
...
plt.xlabel('X Position')
plt.ylabel('Y Position')
plt.savefig("D:\\西工大\\2024秋\\大论文\\图库\\第三章\\svg\\pPO目标集划分.svg", dpi=600, format="svg")
plt.grid()
plt.show()
...
plt.ylabel('Y Position')
plt.grid()
plt.savefig("D:\\西工大\\2024秋\\大论文\\图库\\python画图\\test.png", dpi=600,format="png")
plt.show()
