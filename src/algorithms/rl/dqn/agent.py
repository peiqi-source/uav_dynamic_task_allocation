...
class DQNAgent:
    ...
    def _build_network(self, network_config: DQNNetworkConfig) -> nn.Module:
        """Build the configured Q network."""
        if network_config.network_type == "dqn":
            return DQNNetwork(network_config)

        if network_config.network_type == "attention_dqn":
            from uav_dynamic_task_allocation.algorithms.rl.dqn.attention_network import (
                AttentionDQNNetwork,
            )

            return AttentionDQNNetwork(network_config)

        raise DQNAgentError(
            f"Unsupported network_type: {network_config.network_type}"
        )
