...
class EnvConfig:
    ...
    def validate(self) -> None:
        ...
        if self.simulation_mode not in {"engagement_training", "full_mission"}:
            raise EnvConfigError(
                f"Invalid simulation_mode: {self.simulation_mode}. "
                "Available modes are: engagement_training, full_mission."
            )
    ...
    def with_inferred_bounds(
        ...
        new_config = EnvConfig(
            ...
            enforce_weapon_range=self.enforce_weapon_range,
            update_uav_position_after_attack=self.update_uav_position_after_attack,
        )
        new_config.validate()
    ...
    env_config = EnvConfig(
        ...
        simulation_mode=str(
            get_config_value(
                config,
                "env.simulation_mode",
                default="engagement_training",
            )
        ),
        enforce_weapon_range=bool(
            get_config_value(
                config,
                "env.enforce_weapon_range",
                default=False,
            )
        ),
        update_uav_position_after_attack=bool(
            get_config_value(
                config,
                "env.update_uav_position_after_attack",
                default=True,
            )
        ),
    )
