...
class DroneBattleEnv:
    ...
    def step(self, action: dict[str, int]) -> StepResult:
        ...
        if (
                execution_result.is_valid_action
                and self.env_config.update_uav_position_after_attack
        ):
            uav.position = deepcopy(target.position)
    ...
    def _execute_attack(
        ...
        if self.env_config.enforce_weapon_range and not uav.can_reach(target):
